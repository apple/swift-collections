...
final class ShareableDictionarySmokeTests: CollectionTestCase {
  func testDummy() {
    let map = ShareableDictionary(
  ...
  func testTriggerOverwrite1() {
    ...
    var res2: ShareableDictionary<Int, String> = [:]
    res2[1] = "a" // in-place
    res2[2] = "b" // in-place

    var res3: ShareableDictionary<Int, String> = res2
  ...
  func testTriggerOverwrite2() {
    var res1: ShareableDictionary<Collider, String> = [:]
    ...
    var res2: ShareableDictionary<Collider, String> = res1
  ...
  func testTriggerOverwrite3() {
    ...
    // Populating `map1`
    var map1: ShareableDictionary<Collider, String> = [:]
    for index in 0..<upperBound {
      map1[Collider(index)] = "+\(index)"
    }

    // Populating `map2`
    var map2: ShareableDictionary<Collider, String> = map1
    for index in 0..<upperBound {
    ...
    var map3: ShareableDictionary<Collider, String> = map2
    for index in 0..<upperBound {
  ...
  func testCollisionNodeNotEqual() {
    let map: ShareableDictionary<Collider, Collider> = [:]
  ...
  func testCompactionWhenDeletingFromHashCollisionNode1() {
    ...
    expectEqual(ShareableDictionary([
    ...
    expectEqual(
      ShareableDictionary([Collider(12, 1): Collider(12, 1)]),
      res3)
  ...
  func inferSize<Key, Value>(_ map: ShareableDictionary<Key, Value>) -> Int {
    var size = 0
  ...
  func testIteratorEnumeratesAllIfCollision() {
    ...
    // empty map
    var map3: ShareableDictionary<Collider, String> = map1
    for index in 0..<upperBound {
  ...
  func testIteratorEnumeratesAllIfNoCollision() {
    ...
    var map1: ShareableDictionary<Int, String> = [:]
    for index in 0..<upperBound {
      map1[index] = "+\(index)"
    }
  ...
  func doIterate<Key: Hashable, Value>(
    _ map1: ShareableDictionary<Key, Value>
  ) {
    var count = 0
    for _ in map1 {
      count = count + 1
  ...
  func testIteratorEnumeratesAll() {
    let map1: ShareableDictionary<Collider, String> = [
      Collider(11, 1): "a",
      Collider(12, 1): "a",
      Collider(32769): "b"
    ]

    var map2: ShareableDictionary<Collider, String> = [:]
  ...
  func test_indexForKey_hashCollision() {
    ...
    let map: ShareableDictionary<Collider, String> = [
      a: "a",
      b: "b",
      c: "c",
    ]
    ...
    typealias Index = ShareableDictionary<Collider, String>.Index
  ...
  func test_indexForKey_exhaustIndices() {
    var map: ShareableDictionary<Collider, Int> = [:]
