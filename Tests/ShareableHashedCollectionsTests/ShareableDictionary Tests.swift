...
extension ShareableDictionary: DictionaryAPIExtras {}

class ShareableDictionaryTests: CollectionTestCase {
  func test_empty() {
    let d = ShareableDictionary<String, Int>()
  ...
  func test_remove_update_basics() throws {
    var d = ShareableDictionary<String, Int>()
  ...
  func test_collisions() throws {
    var d = ShareableDictionary<Collider, Int>()
  ...
  func test_Sequence_basic() {
    var d: ShareableDictionary<Int, Int> = [1: 2]
    var it = d.makeIterator()
  ...
  func test_BidirectionalCollection_random100() {
    let d = ShareableDictionary<Int, Int>(
  ...
  func test_Encodable() throws {
    ...
    if #available(macOS 12.3, iOS 15.4, watchOS 8.5, tvOS 15.4, *) {
      let d3: ShareableDictionary<FancyDictionaryKey, Int16> = [
    ...
    let d4: ShareableDictionary<BoringDictionaryKey, UInt8> = [
  ...
  func test_Decodable() throws {
    typealias PD<Key: Hashable, Value> = ShareableDictionary<Key, Value>
  ...
  func test_CustomReflectable() {
    do {
      let d: ShareableDictionary<Int, Int> = [1: 2, 3: 4, 5: 6]
  ...
  func test_Equatable_Hashable() {
    let samples: [[ShareableDictionary<Int, Int>]] = [
      [[:], [:]],
      [[1: 100], [1: 100]],
      [[2: 200], [2: 200]],
      [[3: 300], [3: 300]],
  ...
  func test_CustomStringConvertible() {
    let a: ShareableDictionary<RawCollider, Int> = [:]
    expectEqual(a.description, "[:]")

    let b: ShareableDictionary<RawCollider, Int> = [
      RawCollider(0): 1
    ]
    expectEqual(b.description, "[0#0: 1]")

    let c: ShareableDictionary<RawCollider, Int> = [
      RawCollider(0): 1,
      RawCollider(2): 3,
      RawCollider(4): 5,
    ]
  ...
  func test_CustomDebugStringConvertible() {
    let a: ShareableDictionary<Int, Int> = [:]
    expectEqual(
      a.debugDescription,
      "ShareableDictionary<Int, Int>([:])")

    let b: ShareableDictionary<Int, Int> = [0: 1]
    expectEqual(
      b.debugDescription,
      "ShareableDictionary<Int, Int>([0: 1])")

    let c: ShareableDictionary<Int, Int> = [0: 1, 2: 3, 4: 5]
    let cd = c.map { "\($0.key): \($0.value)"}.joined(separator: ", ")
    expectEqual(
      c.debugDescription,
      "ShareableDictionary<Int, Int>([\(cd)])")
  }


  func test_index_descriptions() {
    let a: ShareableDictionary = [
      RawCollider(1, "1"): 1,
      RawCollider(2, "21"): 2,
      RawCollider(3, "22"): 3,
    ]
  ...
  func test_updateValueForKey_fixtures() {
    withEachFixture { fixture in
      withEvery("isShared", in: [false, true]) { isShared in
        withLifetimeTracking { tracker in
          var d: ShareableDictionary<LifetimeTracked<RawCollider>, LifetimeTracked<Int>> = [:]
  ...
  func test_updateValueForKey_fixtures_tiny() {
    ...
    withEachFixture { fixture in
      withEvery("isShared", in: [false, true]) { isShared in
        withLifetimeTracking { tracker in
          var d: ShareableDictionary<LifetimeTracked<RawCollider>, Empty> = [:]
      ...
      context.withTrace("count: \(count), generator: \(generator)") {
        withEvery("isShared", in: [false, true]) { isShared in
          withLifetimeTracking { tracker in
            ...
            var d: ShareableDictionary<LifetimeTracked<G.Key>, LifetimeTracked<G.Value>> = [:]
  ...
  func test_subscript_modify_basics() {
    func check<G: DataGenerator>(count: Int, generator: G) where G.Value == Int {
      context.withTrace("count: \(count), generator: \(generator)") {
        var d: ShareableDictionary<G.Key, G.Value> = [:]
  ...
  func test_mapValues_basics() {
    ...
    let d = ShareableDictionary(uniqueKeysWithValues: items)
  ...
  //      "d": 3,
  //    ]
  //    let d = ShareableDictionary(items, uniquingKeysWith: +)
  //    expectEqualElements(d, [
  //      (key: "a", value: 5),
  //      (key: "b", value: 2),
  //      (key: "c", value: 1),
  ...
  //    ]
  //    let d = ShareableDictionary<Int, [String]>(
  //      grouping: items,
  //      by: { $0.count })
  //    expectEqualElements(d, [
  ...
  func test_ExpressibleByDictionaryLiteral() {
    let d0: ShareableDictionary<String, Int> = [:]
  ...
  func test_subscript_offset() {
    withEvery("count", in: 0 ..< 30) { count in
      withLifetimeTracking { tracker in
        let (d, _, _) = tracker.shareableDictionary(keys: 0 ..< count)
        withEvery("offset", in: 0 ..< count) { offset in
          let item = d[ShareableDictionaryIndex(value: offset)]
          // NOTE: uses the actual order `d.keys`
...
//            var d: ShareableDictionary<LifetimeTracked<Int>, LifetimeTracked<Int>> = [:]
//            withEvery("offset", in: 0 ..< count) { offset in
//              withHiddenCopies(if: isShared, of: &d) { d in
//                d[keys[offset]] = values[offset]
...
//    }
  ...
  //    let d = ShareableDictionary(uniqueKeys: 0 ..< 30, values: 100 ..< 130)
