...
class ShareableDictionaryKeysTests: CollectionTestCase {
  ...
  func test_descriptions() {
    ...
    if d.first!.key == "a" {
      ...
      expectEqual(
        ...
        "ShareableDictionary<String, Int>.Keys([\"a\", \"b\"])")
    } else {
      expectEqual(d.keys.description, "[b, a]")
      expectEqual(
        d.keys.debugDescription,
        "ShareableDictionary<String, Int>.Keys([\"b\", \"a\"])")
    }
  }

  func test_contains() {
    withEverySubset("a", of: testItems) { a in
      let x = ShareableDictionary<RawCollider, Int>(
  ...
  func test_intersection_exhaustive() {
    withEverySubset("a", of: testItems) { a in
      ...
      withEverySubset("b", of: testItems) { b in
        let y = ShareableDictionary<RawCollider, Int>(
          uniqueKeysWithValues: b.lazy.map { ($0, -$0.identity - 1) })
        let v = Set(b)
        let z = ShareableSet(b)
