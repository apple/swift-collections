...
#if DEBUG // These unit tests need access to Uniqued internals
import XCTest
import CollectionsTestSupport
@_spi(Testing) @testable import UniquedModule

class HashTableTests: CollectionTestCase {
  func test_capacity() {
    withEvery("capacity", in: 0 ..< 1000) { capacity in
      let scale = _UnsafeHashTable.scale(forCapacity: capacity)
      let maximumCapacity = _UnsafeHashTable.maximumCapacity(forScale: scale)
      let minimumCapacity = _UnsafeHashTable.minimumCapacity(forScale: scale)

      if scale == 0 {
        expectEqual(minimumCapacity, 0)
        expectEqual(maximumCapacity, _UnsafeHashTable.maximumUnhashedCount)
        expectGreaterThanOrEqual(maximumCapacity, capacity)
      } else {
        let bucketCount = 1 &<< scale
        expectGreaterThanOrEqual(scale, _UnsafeHashTable.minimumScale)
        expectLessThan(scale, _UnsafeHashTable.maximumScale)
        expectGreaterThanOrEqual(maximumCapacity, capacity)
  ...
  func test_Storage_create() {
    let s5 = _HashTableStorage.create(scale: 5)
    expectEqual(s5.header.scale, 5)
    expectEqual(s5.header.reservedScale, 0)
    expectTrue(s5.description.starts(with: "_HashTableStorage(scale: 5, reservedScale: 0, bias: 0, seed: "))
    expectTrue(s5.header.description.starts(with: "_HashTableHeader(scale: 5, reservedScale: 0, bias: 0, seed: "))
    s5.read { hashTable in
      for i in 0 ..< 32 {
        let bucket = _Bucket(offset: i)
        expectNil(hashTable[bucket], "\(bucket.offset)")
      }
    }

    let s6 = _HashTableStorage.create(scale: 6)
    expectEqual(s6.header.scale, 6)
    expectEqual(s6.header.reservedScale, 0)
    expectTrue(s6.description.starts(with: "_HashTableStorage(scale: 6, reservedScale: 0, bias: 0, seed: "))
    expectTrue(s6.header.description.starts(with: "_HashTableHeader(scale: 6, reservedScale: 0, bias: 0, seed: "))
  ...
  func test_Storage_read() {
    ...
    s5.read { hashTable in
      ...
      expectTrue(hashTable.description.starts(with: "_UnsafeHashTable(scale: 5, reservedScale: 0, bias: 0, seed: "))
    }
    let s6 = _HashTableStorage.create(scale: 6)
    s6.read { hashTable in
      ...
      expectTrue(hashTable.description.starts(with: "_UnsafeHashTable(scale: 6, reservedScale: 0, bias: 0, seed: "))
    }
  }
  ...
  func test_counts() {
    ...
    let s12 = _HashTableStorage.create(scale: 12)
  ...
  func test_bucket_after() {
    withEvery("scale", in: [5, 6, 12]) { scale in
      let storage = _HashTableStorage.create(scale: scale)
      storage.read { hashTable in
        withEvery("offset", in: 0 ..< hashTable.bucketCount) { offset in
          let input = _Bucket(offset: offset)
          let expected = _Bucket(offset: offset == hashTable.bucketCount - 1 ? 0 : offset + 1)
  ...
  func test_bucket_before() {
    withEvery("scale", in: [5, 6, 12]) { scale in
      ...
      storage.read { hashTable in
        withEvery("offset", in: 0 ..< hashTable.bucketCount) { offset in
          let input = _Bucket(offset: offset)
          let expected = _Bucket(offset: offset == 0 ? hashTable.bucketCount - 1 : offset - 1)
  ...
  func test_coordinates_for_bucket() {
    withEvery("scale", in: [5, 6, 12]) { scale in
      ...
      storage.read { hashTable in
        withEvery("offset", in: 0 ..< hashTable.bucketCount) { offset in
          let (word, bit) = hashTable.position(of: _Bucket(offset: offset))
    ...
    withEvery("scale", in: [5, 6, 12]) { scale in
      ...
      storage.update { hashTable in
        for offset in 0 ..< bucketCount {
          let bucket = _Bucket(offset: offset)
  ...
  ) -> (storage: _HashTableStorage, contents: [Int?]) {
    let bucketCount = 1 << scale
  ...
  func withSampleHashTable<R>(
    ...
    let (storage, contents) = sampleStorage(
      scale: scale,
  ...
  func sampleStorage(scale: Int) -> (storage: _HashTableStorage, contents: [Int?]) {
    sampleStorage(scale: scale, chainLength: (1 << scale) - 1, holeLength: 1, startingAt: 0)
  }
  ...
  func withSampleHashTable<R>(
    ...
    let (storage, contents) = sampleStorage(scale: scale)
    return try storage.update { hashTable in
      try body(hashTable, contents)
    }
  }
  ...
  func test_bucketIterator_read() {
    withEvery("scale", in: [5, 6, 7, 8, 12]) { scale in
      withSampleHashTable(scale: scale) { hashTable, contents in
        ...
        withEvery("iteration", in: 0 ..< 2) { _ in
          withEvery("offset", in: 0 ..< hashTable.bucketCount) { offset in
            let expected = contents[offset]
            expectEqual(it.currentBucket, _Bucket(offset: offset))
  ...
  func test_bucketIterator_start_in_middle() {
    withEvery("scale", in: [5, 6, 7, 8, 12]) { scale in
      withSampleHashTable(scale: scale) { hashTable, contents in
        var expected = hashTable.bucketIterator(startingAt: _Bucket(offset: 0))
        ...
        withEvery("start", in: 0 ..< hashTable.bucketCount) { start in
          let bucket = _Bucket(offset: start)
        ...
        withEvery("holeLength", in: [1, 2, 3]) { holeLength in
          withEvery("startOffset", in: 0 ... chainLength) { startOffset in
            withSampleHashTable(
              ...
              var offset = 0
              var it = hashTable.bucketIterator(startingAt: _Bucket(offset: offset))
              let step = 2
  ...
  func test_Storage_copy() {
    withEvery("scale", in: [5, 6, 7, 8, 12]) { scale in
      let (s1, contents) = sampleStorage(scale: scale)
      let s2 = s1.copy()
      s1.read { original in
        s2.read { copy in
          ...
          var it = copy.bucketIterator(startingAt: _Bucket(offset: 0))
