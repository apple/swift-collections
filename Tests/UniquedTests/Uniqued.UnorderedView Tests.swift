...
class UniquedUnorderedViewTests: CollectionTestCase {
    ...
    withEvery("count", in: 0 ..< 20) { count in
      withEvery("dupes", in: 1 ... 3) { dupes in
        withLifetimeTracking { tracker in
          ...
          var set = Uniqued<MinimalBase<LifetimeTracked<Int>>>()
  ...
  func test_unordered_update() {
    withEvery("count", in: 0 ..< 20) { count in
      withEvery("dupes", in: 1 ... 3) { dupes in
        withLifetimeTracking { tracker in
          ...
          var set = Uniqued<MinimalMutableBase<LifetimeTracked<Int>>>()
          withEvery("offset", in: input.indices) { offset in
            let item = input[offset]
    ...
    // Check CoW copying behavior
    do {
      var set = Uniqued<MinimalMutableBase<Int>>(0 ..< 30)
      let copy = set
