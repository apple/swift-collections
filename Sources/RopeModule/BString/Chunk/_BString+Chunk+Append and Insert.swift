...
extension _BString.Chunk {
  ...
  mutating func append(from ingester: inout _BString.Ingester) -> Self? {
    let desired = _BString.Ingester.desiredNextChunkSize(
...
extension _BString.Chunk {
  ...
  mutating func insertAll(
    from ingester: inout _BString.Ingester,
    at index: String.Index
  ) -> States? {
  ...
  enum InsertResult {
    case inline(States?)
    case split(spawn: _BString.Chunk, endStates: States?)
    case large
  }
  ...
  mutating func insert(
    ...
    let desired = _BString.Ingester.desiredNextChunkSize(remaining: sum)
