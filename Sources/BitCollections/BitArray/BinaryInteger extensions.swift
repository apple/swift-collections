...
extension BitArray {
  @inlinable
  package func _convert<T: BinaryInteger>(
    to type: T.Type
  ) -> (
    value: T,
    isNegative: Bool
    ...
    var value: T = .zero
    let isNegative = self._foreachTwosComplementWordDownward(
      isSigned: T.isSigned
    ) { _, word in
      value <<= UInt.bitWidth
      value |= T(truncatingIfNeeded: word)
      return true
    }
  ...
  @inlinable
  package func _convertTruncating<T: BinaryInteger>(to type: T.Type) -> T {
    self._convert(to: T.self).value
  }

  @inlinable
  package func _convertExactly<T: BinaryInteger>(to type: T.Type) -> T? {
    let (value, isNegative) = self._convert(to: T.self)
    guard isNegative == (value < 0) else { return nil }
    let words = value.words
    var equal = true
    _ = self._foreachTwosComplementWordDownward(isSigned: T.isSigned) { i, word in
      assert(equal)
      let w = (
  ...
  @inlinable
  package func _convertForced<T: BinaryInteger>(to type: T.Type) -> T {
    guard let value = _convertExactly(to: T.self) else {
      fatalError("""
