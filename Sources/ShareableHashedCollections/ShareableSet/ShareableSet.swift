...
///
/// `ShareableSet` has the same functionality as a standard `Set`, and
...
///
/// However, `ShareableSet` is optimizing specifically for use cases that
...
/// copy of the set's storage. `ShareableSet` takes a different approach: it
...
/// value, `ShareableSet` is able to simply link the unchanged parts of the
...
/// `ShareableSet` creates, destroys and resizes individual nodes as needed,
...
/// a `ShareableSet` is expected to be a constant factor slower than a standard
...
public struct ShareableSet<Element: Hashable> {
  @usableFromInline
  internal typealias _Node = ShareableHashedCollections._Node<Element, Void>
