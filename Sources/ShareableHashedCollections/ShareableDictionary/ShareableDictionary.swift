...
/// collection.
///
/// `ShareableDictionary` has the same functionality as a standard
...
///
/// However, `ShareableDictionary` is optimizing specifically for use cases
...
/// `ShareableDictionary` takes a different approach: it organizes its members
...
/// `ShareableDictionary` is able to simply link the unchanged parts of the
...
/// `ShareableDictionary` creates, destroys and resizes individual nodes as
...
/// a `ShareableDictionary` is expected to be a constant factor slower than a
...
public struct ShareableDictionary<Key: Hashable, Value> {
  @usableFromInline
...
extension ShareableDictionary {
  ...
  /// dictionary (`"Cerise"`).
  ///
  ///     var hues: ShareableDictionary = ["Heliotrope": 296, "Coral": 16, "Aquamarine": 156]
  ///     print(hues["Coral"])
  ///     // Prints "Optional(16)"
  ///     print(hues["Cerise"])
  ///     // Prints "nil"
  ...
  ///
  ///     var responseMessages: ShareableDictionary = [
  ///         200: "OK",
  ...
  ///     var letterCounts: ShareableDictionary<Character, Int> = [:]
