...
extension ShareableDictionary {
  ...
  ) rethrows -> ShareableDictionary<Key, T> {
    let transformed = try _root.mapValues { try transform($0.value) }
    return ShareableDictionary<Key, T>(_new: transformed)
  }
  ...
  /// `Int` value.
  ///
  ///     let data: ShareableDictionary = ["a": "1", "b": "three", "c": "///4///"]
  ///
  ...
  public func compactMapValues<T>(
    ...
    return ShareableDictionary<Key, T>(_new: result.finalize(.top))
  }
}
