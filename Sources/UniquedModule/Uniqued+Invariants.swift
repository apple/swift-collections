...
extension Uniqued {
  ...
  public func _checkInvariants() {
    if _storage == nil {
      precondition(_elements.count <= _UnsafeHashTable.maximumUnhashedCount,
    ...
    for index in _elements.indices {
      ...
      let (found, _, offset) = _find(item)
      precondition(found, "Element '\(item)' not found in hash table")
      precondition(
        _index(at: offset) == index,
