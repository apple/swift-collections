...
///
/// `PersistentSet` has the same functionality as a standard `Set`, and
...
///
/// However, `PersistentSet` is optimizing specifically for use cases that
...
/// copy of the set's storage. `PersistentSet` takes a different approach: it
...
/// value, `PersistentSet` is able to simply link the unchanged parts of the
...
///
///     var set = PersistentSet(0 ..< 10_000)
///     let copy = set
...
/// `PersistentSet` creates, destroys and resizes individual nodes as needed,
...
/// a `PersistentSet` is expected to be a constant factor slower than a standard
