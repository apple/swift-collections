...
///
/// Each node at an even level in the tree is less than all its descendants, while each node at an odd level
/// in the tree is greater than all of its descendants.
public struct MinMaxHeap<Element: Comparable> {
    ...
    private mutating func _trickleDown(startingAt index: Int) {
        // Figure out if we're on an even or odd level
        let levelIsMin = _minMaxHeapIsMinLevel(index + 1)
...
extension MinMaxHeap {
    /// Initializes a heap from a collection.
    ///
