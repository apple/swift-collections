...
extension RigidDeque where Element: ~Copyable {
  ...
  public mutating func insert<E: Error>(
    ...
    precondition(capacity >= 0, "Cannot insert a negative number of items")
    precondition(capacity <= freeCapacity, "RigidDeque capacity overflow")
    try _handle.uncheckedInsert(capacity: capacity, at: index, initializingWith: body)
  }
}
...
extension RigidDeque where Element: ~Copyable {
...
#if COLLECTIONS_UNSTABLE_CONTAINERS_PREVIEW
  /// Inserts at most `capacity` items generated by a producer into this deque,
  ...
  /// either reaching `capacity`, or the producer hitting its end, or
  /// throwing an error. If the producer has more than `capacity` items left in
  /// its underlying sequence, then extra items remain available after this
  /// method returns.
  ///
  /// If the operation inserts fewer than `capacity` items, then it results in
  ...
  ///
  /// - Parameters:
  ///    - capacity: The maximum number of items to insert into the deque.
  ///    - index: The position at which to insert the new items.
  ///       `index` must be a valid index in the deque.
  ///    - producer: A producer that generates the items to append.
  ///
  /// - Complexity: O(`self.count` + `capacity`)
  @_alwaysEmitIntoClient
  public mutating func insert<
  ...
  >(
    ...
    try insert(maximumCount: maximumCount, at: index) { target throws(P.ProducerError) in
...
extension RigidDeque /* where Element: Copyable */ {
  ...
  >(
    ...
    insert(maximumCount: newCount, at: index) { target in
  ...
  internal mutating func _insertCollection(
    ...
    self.insert(maximumCount: newCount, at: index) { target in
      while !target.isFull {
