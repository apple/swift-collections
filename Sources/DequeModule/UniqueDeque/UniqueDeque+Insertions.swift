...
extension UniqueDeque where Element: ~Copyable {
#if COLLECTIONS_UNSTABLE_CONTAINERS_PREVIEW
  /// Inserts at most `maximumCount` items generated by a producer into this
  ...
  /// throwing an error. If the producer has more than `maximumCount` items
  /// left in its underlying sequence, then extra items remain available after
  /// this method returns.
  ///
  /// If the operation inserts fewer than `maximumCount` items, then it results
  ...
  ///
  /// - Complexity: O(`self.count` + `maximumCount`) when amortized over many
  ///     similar invocations on the same deque.
  @_alwaysEmitIntoClient
  public mutating func insert<
