...
extension UniqueArray where Element: ~Copyable {
  ...
  /// Replaces the specified range of elements by at most `maxCount` items
  ...
  /// either reaching `maxCount`, or the producer hitting its end, or
  /// throwing an error. If the producer has more than `maxCount` items left in
